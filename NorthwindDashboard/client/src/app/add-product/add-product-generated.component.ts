...
export class AddProductGenerated implements AfterViewInit, OnInit, OnDestroy {
  ...
  load() {
    this.northwind.getSuppliers(null, (<any>this.form0.fields.find(f => f.property == 'SupplierID')).pageSize, 0, null, null, null, true)
    ...
    this.northwind.getCategories(null, (<any>this.form0.fields.find(f => f.property == 'CategoryID')).pageSize, 0, null, null, null, true)
    .subscribe((result: any) => {
  ...
  form0LoadData(event: any) {
    if (event.property == 'SupplierID') {
          this.northwind.getSuppliers(`${event.filter}`, event.top, event.skip, `${event.orderby}`, null, null, true)
      .subscribe((result: any) => {
          this.getSuppliersResult = result.value;
    ...
    if (event.property == 'CategoryID') {
          this.northwind.getCategories(`${event.filter}`, event.top, event.skip, `${event.orderby}`, null, null, true)
      .subscribe((result: any) => {
          this.getCategoriesResult = result.value;
