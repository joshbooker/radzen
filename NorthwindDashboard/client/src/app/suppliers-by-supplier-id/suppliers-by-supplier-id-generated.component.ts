...
export class SuppliersBySupplierIdGenerated implements AfterViewInit, OnInit, OnDestroy {
  ...
  load() {
    this.northwind.getSuppliers(`SupplierID eq ${this.parameters.SupplierID}`, this.grid0.allowPaging ? this.grid0.pageSize : null, this.grid0.allowPaging ? 0 : null, null, null, null, this.grid0.allowPaging)
  ...
  grid0LoadData(event: any) {
    this.northwind.getSuppliers(`${event.filter ? event.filter + ' and ' : ''}SupplierID eq ${this.parameters.SupplierID}`, event.top, event.skip, `${event.orderby}`, ``, null, event.top != null && event.skip != null)
    .subscribe((result: any) => {
      this.getSuppliersResult = result.value;
