...
export class OrderDetailsByOrderIdGenerated implements AfterViewInit, OnInit, OnDestroy {
  ...
  load() {
    this.northwind.getNorthwindOrderDetails(`OrderID eq ${this.parameters.OrderID}`, this.grid0.allowPaging ? this.grid0.pageSize : null, this.grid0.allowPaging ? 0 : null, null, `NorthwindOrder,NorthwindProduct`, null, this.grid0.allowPaging)
  ...
  grid0LoadData(event: any) {
    this.northwind.getNorthwindOrderDetails(`${event.filter ? event.filter + ' and ' : ''}OrderID eq ${this.parameters.OrderID}`, event.top, event.skip, `${event.orderby}`, `NorthwindOrder,NorthwindProduct`, null, event.top != null && event.skip != null)
    .subscribe((result: any) => {
      this.getNorthwindOrderDetailsResult = result.value;
