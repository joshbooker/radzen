...
export class AddTerritoryGenerated implements AfterViewInit, OnInit, OnDestroy {
  ...
  load() {
    this.northwind.getRegions(null, (<any>this.form0.fields.find(f => f.property == 'RegionID')).pageSize, 0, null, null, null, true)
  ...
  form0LoadData(event: any) {
    if (event.property == 'RegionID') {
          this.northwind.getRegions(`${event.filter}`, event.top, event.skip, `${event.orderby}`, null, null, true)
      .subscribe((result: any) => {
          this.getRegionsResult = result.value;
