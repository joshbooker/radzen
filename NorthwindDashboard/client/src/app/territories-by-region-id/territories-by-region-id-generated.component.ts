...
export class TerritoriesByRegionIdGenerated implements AfterViewInit, OnInit, OnDestroy {
  ...
  load() {
    this.northwind.getTerritories(`RegionID eq ${this.parameters.RegionID}`, this.grid0.allowPaging ? this.grid0.pageSize : null, this.grid0.allowPaging ? 0 : null, null, `Region`, null, this.grid0.allowPaging)
  ...
  grid0LoadData(event: any) {
    this.northwind.getTerritories(`${event.filter ? event.filter + ' and ' : ''}RegionID eq ${this.parameters.RegionID}`, event.top, event.skip, `${event.orderby}`, `Region`, null, event.top != null && event.skip != null)
    .subscribe((result: any) => {
      this.getTerritoriesResult = result.value;
