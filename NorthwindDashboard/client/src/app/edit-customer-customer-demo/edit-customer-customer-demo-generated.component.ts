...
export class EditCustomerCustomerDemoGenerated implements AfterViewInit, OnInit, OnDestroy {
  ...
  load() {
    ...
    this.northwind.getCustomers(null, (<any>this.form0.fields.find(f => f.property == 'CustomerID')).pageSize, 0, null, null, null, true)
    .subscribe((result: any) => {
    ...
    this.northwind.getCustomerDemographics(null, (<any>this.form0.fields.find(f => f.property == 'CustomerTypeID')).pageSize, 0, null, null, null, true)
    .subscribe((result: any) => {
  ...
  form0LoadData(event: any) {
    if (event.property == 'CustomerID') {
          this.northwind.getCustomers(`${event.filter}`, event.top, event.skip, `${event.orderby}`, null, null, true)
      .subscribe((result: any) => {
          this.getCustomersResult = result.value;
    ...
    if (event.property == 'CustomerTypeID') {
          this.northwind.getCustomerDemographics(`${event.filter}`, event.top, event.skip, `${event.orderby}`, null, null, true)
      .subscribe((result: any) => {
          this.getCustomerDemographicsResult = result.value;
