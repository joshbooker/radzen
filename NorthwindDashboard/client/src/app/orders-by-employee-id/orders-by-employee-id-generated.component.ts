...
export class OrdersByEmployeeIdGenerated implements AfterViewInit, OnInit, OnDestroy {
  ...
  load() {
    this.northwind.getNorthwindOrders(`EmployeeID eq ${this.parameters.EmployeeID}`, this.grid0.allowPaging ? this.grid0.pageSize : null, this.grid0.allowPaging ? 0 : null, null, `Customer,Employee`, null, this.grid0.allowPaging)
  ...
  grid0LoadData(event: any) {
    this.northwind.getNorthwindOrders(`${event.filter ? event.filter + ' and ' : ''}EmployeeID eq ${this.parameters.EmployeeID}`, event.top, event.skip, `${event.orderby}`, `Customer,Employee`, null, event.top != null && event.skip != null)
    .subscribe((result: any) => {
      this.getNorthwindOrdersResult = result.value;
