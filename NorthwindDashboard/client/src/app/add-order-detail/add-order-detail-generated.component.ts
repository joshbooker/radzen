...
export class AddOrderDetailGenerated implements AfterViewInit, OnInit, OnDestroy {
  ...
  load() {
    this.northwind.getNorthwindOrders(null, (<any>this.form0.fields.find(f => f.property == 'OrderID')).pageSize, 0, null, null, null, true)
    ...
    this.northwind.getNorthwindProducts(null, (<any>this.form0.fields.find(f => f.property == 'ProductID')).pageSize, 0, null, null, null, true)
    .subscribe((result: any) => {
  ...
  form0LoadData(event: any) {
    if (event.property == 'OrderID') {
          this.northwind.getNorthwindOrders(`${event.filter}`, event.top, event.skip, `${event.orderby}`, null, null, true)
      .subscribe((result: any) => {
    ...
    if (event.property == 'ProductID') {
          this.northwind.getNorthwindProducts(`${event.filter}`, event.top, event.skip, `${event.orderby}`, null, null, true)
      .subscribe((result: any) => {
          this.getProductsResult = result.value;
