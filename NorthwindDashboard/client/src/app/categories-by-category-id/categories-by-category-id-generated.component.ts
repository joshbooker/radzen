...
export class CategoriesByCategoryIdGenerated implements AfterViewInit, OnInit, OnDestroy {
  ...
  load() {
    this.northwind.getCategories(`CategoryID eq ${this.parameters.CategoryID}`, this.grid0.allowPaging ? this.grid0.pageSize : null, this.grid0.allowPaging ? 0 : null, null, null, null, this.grid0.allowPaging)
  ...
  grid0LoadData(event: any) {
    this.northwind.getCategories(`${event.filter ? event.filter + ' and ' : ''}CategoryID eq ${this.parameters.CategoryID}`, event.top, event.skip, `${event.orderby}`, ``, null, event.top != null && event.skip != null)
    .subscribe((result: any) => {
      this.getCategoriesResult = result.value;
